-- ============================================================================
-- CANDIDATE (pump venues) -- top_traders_solana.candidate.sql with the source
-- table swapped. NOT LIVE, NOT VERIFIED.
-- ============================================================================
-- dex_solana.trades is every decoded swap on every Solana venue ever. A
-- pump.fun mint only ever trades in two places: the bonding curve while it is
-- on it, and PumpSwap after it migrates. Both have their own tables with the
-- same column names, and both are a small fraction of the size of the union.
--
-- Applies ONLY to mints ending in "pump", and only safely to ones that
-- migrated to PumpSwap. Tokens that graduated to Raydium instead -- which is
-- where pump.fun sent them before PumpSwap existed -- would lose every
-- post-migration fill and read as though the wallets stopped trading. Anything
-- routed through a venue outside these two is invisible here.
--
-- So this is a fast path, not a replacement, and it needs the A/B diff to hold
-- on several tokens of different ages before it can be trusted for any of them.
-- The other candidate's rewrites (single scan, day-partitioned price fill) are
...
dex_scan AS (
    SELECT
        ...
        token_sold_amount
    FROM (
        SELECT block_time, tx_id, trader_id, amount_usd, block_month,
               token_bought_mint_address, token_sold_mint_address,
               token_bought_amount, token_sold_amount
        FROM pumpdotfun_solana.trades
        UNION ALL
        SELECT block_time, tx_id, trader_id, amount_usd, block_month,
               token_bought_mint_address, token_sold_mint_address,
               token_bought_amount, token_sold_amount
        FROM pumpswap_solana.trades
    )
